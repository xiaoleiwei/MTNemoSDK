...
/**
 ...
 @param state 参考NemoContentState
 */
...
/**
 ...
 @param type 会控消息
 */
- (void)nemoSDKDidConferenceManagementStateChanged:(NemoConferenceManagerManagementType)type DEPRECATED_MSG_ATTRIBUTE("use nemoSDKDidConferenceManagementStateChanged:info: instead");
...
/** 获取是否开启自定义布局（初始化NemoSDk环境 调用使用） */
@property (readonly, nonatomic, assign) BOOL isCustomLayout;
...
/**
 ...
 @param image 获取的分享内容
 */
- (void)getContentImage:(void (^)(UIImage *image))image;
...
/**
 ...
 @param streamID(设置以后将对应视频流对应到大屏)
 */
- (void)focusOn:(NSString *)streamID;
...
- (NSString *)currentAccountUserProfileDisplayName DEPRECATED_MSG_ATTRIBUTE("use getUserProfile instead");
...
/**
 ...
 视频会议中，聚焦到指定participantId的视频。在网络状况不好时，会优先分配更高的分辨率。
 
 @param participantId 参会者id
 */
- (void)focusVideoStreamWithParticipantId:(NSString *)participantID;
